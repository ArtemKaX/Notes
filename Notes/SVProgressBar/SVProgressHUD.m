...
+ (SVProgressHUD *)HUDForView:(UIView *)view {
	Class hudClass = [SVProgressHUD class];
	NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
	for (UIView *subview in subviewsEnum) {
		if ([subview isKindOfClass:hudClass]) {
			return (SVProgressHUD *)subview;
		}
	}
	return nil;
}

+ (void)setStatus:(NSString *)string forView:(UIView*)view {
	SVProgressHUD *hud = [SVProgressHUD HUDForView:view];
	[hud setStatus:string];
}
...
+ (void)showHUDAddedTo:(UIView *)view {
    SVProgressHUD *hud = [SVProgressHUD HUDForView:view];
	if (hud == nil) {
        hud = [[SVProgressHUD alloc] initWithView:view];
        [view addSubview:hud];
    }
    [hud showProgress:-1 status:nil maskType:SVProgressHUDMaskTypeNone];
}

+ (void)showWithStatus:(NSString *)status forView:(UIView*)view {
    SVProgressHUD *hud = [SVProgressHUD HUDForView:view];
	if (hud == nil) {
        hud = [[SVProgressHUD alloc] initWithView:view];
        [view addSubview:hud];
    }
    [hud showProgress:-1 status:status maskType:SVProgressHUDMaskTypeNone];
}

+ (void)showWithMaskType:(SVProgressHUDMaskType)maskType forView:(UIView*)view {
    SVProgressHUD *hud = [SVProgressHUD HUDForView:view];
	if (hud == nil) {
        hud = [[SVProgressHUD alloc] initWithView:view];
        [view addSubview:hud];
    }
    [hud showProgress:-1 status:nil maskType:maskType];
}
...
+ (void)showWithStatus:(NSString*)status maskType:(SVProgressHUDMaskType)maskType
    ...
    [hud showProgress:-1 status:status maskType:maskType];
}

+ (void)showProgress:(CGFloat)progress forView:(UIView*)view {
    SVProgressHUD *hud = [SVProgressHUD HUDForView:view];
	if (hud == nil) {
        hud = [[SVProgressHUD alloc] initWithView:view];
        [view addSubview:hud];
    }
    [hud showProgress:progress status:nil maskType:SVProgressHUDMaskTypeNone];
}

+ (void)showProgress:(CGFloat)progress status:(NSString *)status forView:(UIView*)view {
    SVProgressHUD *hud = [SVProgressHUD HUDForView:view];
	if (hud == nil) {
        hud = [[SVProgressHUD alloc] initWithView:view];
        [view addSubview:hud];
    }
    [hud showProgress:progress status:status maskType:SVProgressHUDMaskTypeNone];
}

+ (void)showProgress:(CGFloat)progress status:(NSString *)status maskType:(SVProgressHUDMaskType)maskType forView:(UIView*)view {
    SVProgressHUD *hud = [SVProgressHUD HUDForView:view];
	if (hud == nil) {
        hud = [[SVProgressHUD alloc] initWithView:view];
        [view addSubview:hud];
    }
    [hud showProgress:progress status:status maskType:maskType];
}

#pragma mark - Show then dismiss methods

+ (void)showSuccessWithStatus:(NSString *)string forView:(UIView*)view {
	SVProgressHUD * hud = [[SVProgressHUD alloc] initWithView:view];
	[view addSubview:hud];
	[hud showImage:[UIImage imageNamed:@"SVProgressHUD.bundle/success.png"] status:string];
}

+ (void)showErrorWithStatus:(NSString *)string forView:(UIView*)view
{
    SVProgressHUD *hud = [SVProgressHUD HUDForView:view];
	if (hud == nil) {
        hud = [[SVProgressHUD alloc] initWithView:view];
	...
	[hud showImage:[UIImage imageNamed:@"SVProgressHUD.bundle/error.png"] status:string];
}

- (void)showImage:(UIImage *)image status:(NSString *)string {
    [self showImage:image status:string duration:2.0];
}
